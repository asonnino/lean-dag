...
/-! ## The same rule, over the partial schedule

`CatchupPace` is the `ViewPace` incarnation, and the restatement is the
same move the coverage clauses made: off `blk`, over any `T`-authored
block. One genuine strengthening is forced by the partial schedule, and
it is the honest one — over a total schedule, *entering a round* is
trivially meaningful because every round has a build time; here the
clause must say that seeing a round means **reaching** it
(`n ≤ top v`), which is exactly what a real pacemaker's catch-up does.
The conclusion `built v n ≤ t + proc` then reads off a round the
validator did reach. -/
