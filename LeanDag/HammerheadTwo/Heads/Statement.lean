...
/-!
# HH9 — the heads descent

The base protocol's liveness clause, `LiveOn`, discharged for the
paper's own schedule at every leader count (`hammerhead-two.md` §8):
...
* **HH9a, the stretch descent** — a stretch of consecutive decided slots
  with a committed top decides everything a wave below the top.
* **HH9b, heads decide** — `waveLength` consecutive good-led heads decide
  every slot up to a wave below the first and commit it.
* **HH9c, live from heads** — a run of heads with gap `c₀` makes every
  count's schedule live with gap `c₀`.
* **HH9d, round-robin has runs of heads** — the pigeonhole.
* **HH9e, round-robin is live** — at every count, with gap
  `n + waveLength − 1`.
...
/-- **HH9b, heads decide**: under `Sched m`, `waveLength` consecutive
...
/-- **HH9c, live from heads**: a run of good-led heads with gap `c₀`, for
...
/-- **HH9d, round-robin has runs of heads**: on `n` validators, for every
...
/-- **HH9e, round-robin is live**: a live rule with descent laws at
