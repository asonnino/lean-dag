...
(`hammerhead-two.md` §6): the committed sequence read from any two runs
...
Stated of partial runs, the only runs there are (`hammerhead-two.md`
...
* **HH5a, the ledger is agreed** — range by range, and as one list, as
  far as both runs reach.
* **HH5b, the ledger grows** — the ledger to a lower height is a prefix
  of the ledger to a higher one.
* **HH5c, integrity** — no block appears twice, to any height the run
  reaches.
...
/-- **HH5a, the ledger is agreed**: two runs over one universe read the
...
/-- **HH5b, the ledger grows**: to a lower height it is a prefix of itself
...
/-- **HH5c, integrity**: no block appears twice in the ledger, to any
