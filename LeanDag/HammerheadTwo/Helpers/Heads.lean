...
/-- **What HH9b needs.** For a slot `κ` at round `r` and the head `h := m * (r + w)` of round
...
/-- **HH9b.** Heads of rounds `ρ + w, …, ρ + 2w − 1` `T`-led (with `T` from `goodLeaders`)
...
/-- **HH9b′, subtraction-free.** Heads of rounds `ρ, …, ρ + w − 1` `T`-led, waves under `N`:
