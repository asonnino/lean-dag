...
/-!
# Hammerhead 2.0: the run

The object safety and liveness are stated on (`hammerhead-two.md` §5).
...
A `PartialRun` is closed up to a configuration height `K`, through each
range's anchor: what a validator holds mid-execution, and what the
configuration sequence is a function of. A `ConfigRun` is total and its
ranges are closed through the end of the anchor's round, so that each
range's ledger is defined.
...
determined, and the ranges of configurations below `K` are decided
through their anchors.

`closed` is the paper's `TryDecide`: every slot of the range, up to the
anchor, decided against the configuration's schedule. `anchor_commits`
...
/-- **A total run**: every configuration determined, every range decided
through the end of its anchor's round. -/
structure ConfigRun (R : BaseRule Validator BlockId Payload) (P : Params)
    (getLeader : ℕ → Validator) (hk : Keyed getLeader P.maxLeaders)
    (upd : UpdateRule R) (U : R.Universe) (V : R.View U) where
  /-- The round after which configuration `k` is in force. -/
  start : ℕ → ℕ
  /-- The leader count of configuration `k`. -/
  count : ℕ → ℕ
  ...
  count_pos : ∀ k, 0 < count k
  count_le : ∀ k, count k ≤ P.maxLeaders
  /-- Every slot of the range, through the end of the anchor's round, is
  decided against the configuration's schedule. -/
  closed : ∀ k κ, start k < κ / count k → κ / count k ≤ start (k + 1) →
    R.Decided (Sched getLeader hk (count k) (count_pos k) (count_le k)) V κ (vdct k κ)
  anchor_commits : ∀ k,
    (∃ A, vdct k (anchor k) = some A) ∧ start k + P.interval < anchor k / count k
  anchor_least : ∀ k κ, κ < anchor k → start k + P.interval < κ / count k → vdct k κ = none
  start_succ : ∀ k, start (k + 1) = anchor k / count k
  update : ∀ k A, vdct k (anchor k) = some A →
    (count (k + 1), backoff (k + 1)) = upd (count k) (backoff k) U A
...
/-- The schedule of configuration `k`. -/
abbrev ConfigRun.sched (Rn : ConfigRun R P getLeader hk upd U V) (k : ℕ) :
    Slots Validator :=
  Sched getLeader hk (Rn.count k) (Rn.count_pos k) (Rn.count_le k)
...
def ConfigRun.rangeLedger (Rn : ConfigRun R P getLeader hk upd U V) (k : ℕ) :
    List BlockId :=
...
def ConfigRun.ledgerUpto (Rn : ConfigRun R P getLeader hk upd U V) (K : ℕ) :
    List BlockId :=
