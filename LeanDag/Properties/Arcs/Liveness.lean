...
theorem certifiesAt_of_rebased (hloc : sp.Local) {U U' : R.Universe} {G R₀ : ℕ}
    (h : RebasedAbove R U U' G R₀) {T : Finset Validator} {r κ : ℕ} {L : BlockId}
    (hr : R₀ ≤ r) (hG : G ≤ r) (hL : L ∈ R.ids U)
    (hLr : (R.block U L).round = r) (hc : sp.certifiesAt U T r κ L) :
    sp.certifiesAt U' T (r - G) κ L := by
  intro v hv c hc' hcc hcr
  obtain ⟨hcU, hround⟩ := h.of_mem' hc' (by omega)
  have hcrU : (R.block U c).round = r + sp.waveAt κ := by omega
  have hccU : (R.block U c).creator = v := by
    rw [← h.creator c hcU (by omega)]; exact hcc
  exact (hloc h c L κ hcU (by omega) hL (by rw [hLr]; omega)).mpr
...
theorem exists_decided_of_sustains {rel : Reliability Validator}
    ...
    (hR₀ : R₀ ≤ S.slotRound k)
    (hpop : ∀ n, S.slotRound k ≤ n → n ≤ S.slotRound k + sp.waveAt (S.kind k) →
      PopulatedOn R U T n)
    (hcert : ∀ L, R.IsCandidate S U k L → sp.certifiesAt U T (S.slotRound k) (S.kind k) L)
    (hV' : CoversUpto R V' (S.slotRound k + sp.waveAt (S.kind k))) (hlead : S.leader k ∈ T) :
