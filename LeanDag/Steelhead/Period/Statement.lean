...
(`steelhead.md` §5). Nineteen claims:
...
* **SH10j, a window resolves an asynchronous slot of every candidate**:
  ...
  one round short;
* **SH10k, the control schedule enumerates the control rounds**: the
...
* **SH10n, every window holds two asynchronous rounds**: the adaptive
...
* **SH10o, the period stays a divisor of the bound**: if the initial
...
* **SH10p, the control slots carry a coin**: the protocol section's
  ...
  period of whatever interval it falls in. With SH5b, the two readings
  agree on every such slot whenever either verdict is direct;
* **SH10q, the gating rule**: the adaptive section's "a slot is
...
* **SH14a, output liveness under the failover**: Theorem 3 (ii) and the
  ...
  last commit lies below it (SH10i), the failover fires at each such
  anchor and the period is `1` from the first, where the run decides
  everything below it (SH9a);
* **SH14b, every slot is decided under the clause**: SH14a with its two
  ...
  boundary, so the run the clause places there is the run of rounds SH14a
...
* **SH14c, output liveness from a good coin and a good run**: SH14a with
  ...
  good coins above that interval are SH14a's run. The form the coin's
...
wavelength on both sides; SH10b, SH10h, SH10i and SH14a to SH14c read it
at the pair's, `wavelength ws wa`, on a schedule whose kinds are the
validator's own sequence's (`adaptiveKind`). SH10i and SH14a to SH14c
...
SH10a assumes `3 ≤ wa`, as SH5a and SH10l do, and SH10b `3 ≤ ws` and
`3 ≤ wa` and a round `N` the record does not reach past; SH10c, SH10h and
SH10i assume `2 ≤ w κ`, what the laws need to carry a verdict from the
...
in range, SH10g's hypotheses; SH10m assumes a positive interval too, so
that interval `0` ends at round `I`; SH10e, SH10g and SH10k read no
record; SH14a assumes `2 ≤ ws ≤ wa` and `3 ≤ wa`, as SH10a does, one slot
...
initial period and the update rule. SH10n asks `1 ≤ k` as SH10f does and
an anchor at round `I` or above, below which the window is shorter than
`I` rounds; SH10o reads no record; SH10p asks a positive interval and
