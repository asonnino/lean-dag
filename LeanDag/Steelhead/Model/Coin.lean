...
slot of round `r` commits directly when the coin lands in the round's good
set `G r`, the validators whose round-`r` block the DAG directly commits
(`GoodCommits`); the quantities below are the probabilities the counting
lemma bounds in `Coin/Statement.lean`, read at any good set and at any
control rule `Ra` and output rule `R`: of one good coin, of `m` bad ones in a
...
the good set `G r` under the uniform coin. -/
noncomputable def commitProb (G : ℕ → Finset Validator) (r : ℕ) : ℝ≥0∞ :=
  (PMF.uniformOfFintype Validator).toOuterMeasure ↑(G r)
...
noncomputable def goodBlocks (G : ℕ → Finset Validator) (wa b : ℕ) {M : ℕ}
    (g : Fin M → Fin wa → Validator) : Finset (Fin M) :=
  open Classical in
  Finset.univ.filter fun j => ∀ i : Fin wa, g j i ∈ G (b + j * wa + i)
...
noncomputable def noCommitProbOn (G : ℕ → Finset Validator) {c : ℕ} (ρ : Fin c → ℕ) : ℝ≥0∞ :=
  (PMF.uniformOfFintype (Fin c → Validator)).toOuterMeasure
    {coins | ∀ i : Fin c, coins i ∉ G (ρ i)}
...
noncomputable def goodIntervals (G : ℕ → Finset Validator) {M c : ℕ} (ρ : Fin M → Fin c → ℕ)
    (g : Fin M → Fin c → Validator) : Finset (Fin M) :=
  open Classical in
  Finset.univ.filter fun j => ∃ i : Fin c, g j i ∈ G (ρ j i)
...
noncomputable def firstGoodInterval (G : ℕ → Finset Validator) {M c : ℕ}
    (ρ : Fin M → Fin c → ℕ) (g : Fin M → Fin c → Validator) : ℕ :=
  open Classical in
  if h : (goodIntervals G ρ g).Nonempty then ((goodIntervals G ρ g).min' h : ℕ) else M
...
/-- **The horizon the blocks need**: the decision round of the last block's last round at a wave
of `wa` rounds, the blocks being `wa · K` rounds long and the last one opening interval
`j₀ + 2 + q · (M − 1)`. -/
def blocksHorizon (I q wa K j₀ M : ℕ) : ℕ :=
  (j₀ + 2 + q * (M - 1)) * I + wa * K + wa - 1

/-- **A period sequence matches what a view derives**: at every interval the view derives a state
for, its control verdicts read at `Ra` and its agreed output at `R` on the schedule the sequence
names, whose kinds are the sequence's own, the sequence's period is the state's. Arbitrary where
the scan has stalled. -/
def Matches (I K : ℕ) [NeZero K] (Ra R : AnchoredRule Validator BlockId Payload ValidWrt Correct)
    (coin known : ℕ → Validator) (upd : UpdateRule BlockId) (k₀ : ℕ)
    (U : BlockUniverse Validator BlockId Payload) (V : View Validator BlockId Payload U)
    (per : ℕ → ℕ) : Prop :=
  ∀ j st, PeriodAt (S := adaptiveSlots coin known I per) I K Ra coin upd k₀ U V R j st →
    per j = st.period

/-- **A view settles a slot at a matching sequence**: it derives the state of the slot's interval,
and decides the slot at `R` on the sequence's schedule. -/
def Settles (I K : ℕ) [NeZero K] (Ra R : AnchoredRule Validator BlockId Payload ValidWrt Correct)
    (coin known : ℕ → Validator) (upd : UpdateRule BlockId) (k₀ : ℕ)
    (U : BlockUniverse Validator BlockId Payload) (V : View Validator BlockId Payload U)
    (per : ℕ → ℕ) (s : ℕ) : Prop :=
  (∃ st, PeriodAt (S := adaptiveSlots coin known I per) I K Ra coin upd k₀ U V R (intervalOf I s)
    st) ∧
  ∃ v, R.Decided (S := adaptiveSlots coin known I per) U V s v
...
undecided at `R` on that sequence's schedule. A sequence matching what the view derives
...
blocks of `K` rounds, and at every round of a block a set of candidates in the record's good set
`good`, fixed by the coins drawn before that round, those of the blocks below and of the
...
def NonAnticipating {M K : ℕ}
    ...
    (good : BlockUniverse Validator BlockId Payload → ℕ → Finset Validator) (I q j₀ : ℕ) : Prop :=
  (∀ g (j : Fin M) (i : Fin K), G g j i ⊆ good (σ g) (blockRound I q j₀ j i)) ∧
...
noncomputable def undecidedProbAgainst
    (Ra R : AnchoredRule Validator BlockId Payload ValidWrt Correct) (wa I q : ℕ) {M K : ℕ}
    [NeZero K] (σ : (Fin M → Fin (wa * K) → Validator) → BlockUniverse Validator BlockId Payload)
    (upd : UpdateRule BlockId) (k₀ : ℕ) (known : ℕ → Validator) (d : Validator) (s : ℕ) :
    ℝ≥0∞ :=
  (PMF.uniformOfFintype (Fin M → Fin (wa * K) → Validator)).toOuterMeasure
    {g | ¬ ∀ (V : View Validator BlockId Payload (σ g)) (per : ℕ → ℕ),
      V.CoversUpto (blocksHorizon I q wa K (intervalOf I s) M) →
      Matches I K Ra R (coinOfBlocks I q (intervalOf I s) g d) known upd k₀ (σ g) V per →
      Settles I K Ra R (coinOfBlocks I q (intervalOf I s) g d) known upd k₀ (σ g) V per s}
