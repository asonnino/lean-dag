...
comes from the **chain verdict**: the asynchronous rule read with the
coin leader on the rounds that carry a coin, anchored on chain commits
only, so that no known-leader slot lies on the chain and nothing the
adversary can hold undecided blocks it. Chain verdicts are never
...
unpredictable-leader clause at that schedule.

**Which rounds carry a coin is fixed by rule, per scan**, and the next
development of the arc supplies that set: a coin exists on the
asynchronous rounds alone, so the scan of an interval reads the
multiples of its own period up to the interval's boundary, and above it
the multiples of the period bound, which are asynchronous whatever the
scan fixes. That reading is the same relation at a sub-schedule of the
one below, so its agreement is the same lemma at that schedule. This is
the reference implementation's reading (`committer.rs`,
`is_control_round`, `compute_chain`), where a round without a coin reads
as a skip, which the anchor search and the scan pass over.
