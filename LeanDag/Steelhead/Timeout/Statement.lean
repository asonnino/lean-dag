...
/-!
# The mistimed leader timeout — statement

The arithmetic of the paper's appendix on a leader timeout smaller than the
link delay (`steelhead.md` §10). Two claims:

* **SH20a, the certificate probability at the top of the vote round** — with
...
* **SH20b, the wait for the next direct commit** — the appendix's `1 / P₂`.
...
SH20b is the series, not an expectation over a process the model carries:
...
appendix states the last of them as its one optimistic step.
...
/-- **SH20a, the certificate probability at a full and a nearly full vote round.** -/
...
/-- **SH20b, the wait for the next direct commit.** -/
