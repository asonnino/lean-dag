...
/-!
# The `3f + 1` pair's coin — statement

The coin at the Mysticeti and Mahi-Mahi pair, the good set Mahi-Mahi's
`goodAt` at `wa` and the counting lemma MM2. A claim numbered like one of
the generic statements of `Coin/Statement.lean` is that statement at this
pair, with MM2 as its floor: `n − f − b` at `wa ≥ 5`, and one committed
candidate at `wa ≥ 4` (SH-MM11b, SH-MM11k, SH-MM15d, the generic claims at
a floor of one).
...
* **SH-MM11a, the commit probability** — on a wave a quorum has populated,
...
* **SH-MM11b, the commit probability at wave four**: at `wa ≥ 4` MM2
...
* **SH-MM11c, the coin names a Byzantine leader with probability `b / n`**:
...
* **SH-MM11d, the run probability** — the positive form the paper's
  ...
  bound is SH-MM11f, and neither implies the other: SH-MM11f bounds the
  chance that no round commits;
* **SH-MM11e, a good coin commits the chain slot** — at any schedule
...
* **SH-MM11f, the tail** — over `m` consecutive populated waves with
  independent coins, no round's coin names a directly committed leader
  with probability at most `((f + |byzantine|) / n)^m`;
* **SH-MM11g, the tail vanishes** — that bound tends to zero, since
  `f + |byzantine| ≤ 2f < n`;
* **SH-MM11h, the adaptive block bound** — the same bound for good sets
...
* **SH-MM15a, the output is live but for a vanishing probability** —
  ...
  interval (SH-MM7a at each scan's schedule) and the states are derived up
  ...
  `1`. Two good blocks in different halves then decide the slot (SH-MM14c),
...
* **SH-MM15b, the tail against an adaptive adversary** — SH-MM15a where the
  ...
  by SH-MM11h. The adversary names, at every round of the blocks, a floor
...
* **SH-MM15c, that tail vanishes** — the bound tends to zero as the number
  of blocks grows, since `n − f − b ≥ 1`;
* **SH-MM15d, the tail at wave four** — SH-MM15a at `4 ≤ wa`, the wave the
  ...
  per populated round (SH-MM11b), so a block of `wa · K` coins holds a bad
...
* **SH-MM15e, the slot is decided almost surely** — Theorem 3's "with
  ...
  its horizon, in SH-MM15a's sense (`Matches`, `Settles`). The coins under which no record decides
  it lie, for every `m`, among those under which the `m`-th leaves it
  undecided, a set of measure at most SH-MM15a's bound, which vanishes
  (SH-MM15c); so they are null. The records are any sequence, the prefixes
  of one execution among them, since the argument reads each on its own;
* **SH-MM11i, the search under the coin** — Theorem 2's asynchronous-floor
  ...
  (SH-MM9a), so over `M` consecutive blocks of `wa` coins above the slot it
...
* **SH-MM11j, the expected wait of the search at period one** — Theorem
  2's expectation above an asynchronous floor, in the form SH-MM11i admits:
  ...
  bad, at most `(1 − p^wa)^m` each (SH-MM11i's count), whose sum is
  `1 / p^wa`. In rounds, `wa` per block, that is the paper's
  `wa / p^wa`, the decision falling within a wave of the good block;
* **SH-MM11k, the expected wait at wave four** — SH-MM11j at `4 ≤ wa`, where
  the counting lemma's weaker form promises one committed candidate per
  populated round (SH-MM11b): the wait is at most `n^wa` blocks in
  expectation, the paper's `1 / p^wa` at its `p = 1/n`;
* **SH-MM11l, a scan's control slots all miss** — the chance that no slot
  of a scan names a committed candidate is at most `((f + b) / n)^c` at
  `c` slots, SH-MM11f at a schedule rather than a stretch, since control
  slots sit at multiples of the period;
* **SH-MM11m, the expected number of scans before one anchors** — the
  asynchronous section's `1 / (1 − (1 − p)^c)` intervals. The scans'
  misses are independent across intervals, so the wait is a geometric
  series in SH-MM11l's bound, and the sum is the inverse of an anchoring
...
* **SH-MM15f, some interval is anchored almost surely** — Theorem 3 (i)'s
  "with probability `1` some scan finds its anchor": SH-MM15e's argument
...
* **SH-MM15g, almost surely against an adaptive adversary** — SH-MM15e where
  ...
  by SH-MM15b at each `m`;
* **SH-MM15h, every slot is decided almost surely** — SH-MM15e for all slots
  ...
  sets of SH-MM15e add up to one. What one execution supplies is a prefix
  for every slot and every `m`;
* **SH-MM15i, a matching sequence exists** — SH-MM15a, SH-MM15e, SH-MM15b and SH-MM15g
  ...
  congruence behind SH-MM10b). So none of those claims is empty for want of
  a sequence.

The blocks' coins are drawn after the record is fixed, as SH-MM11f's are,
and the records of SH-MM15e before the process: the adversary that shapes
the DAG does not see them, which is the coin's unpredictability. SH-MM11h,
SH-MM15b and SH-MM15g weaken that to what the argument needs, an adversary
that answers the draws already made and keeps a floor of committed
candidates the next draw cannot shrink. What SH-MM15a leaves to the network
is what SH-MM14c asks of it, that the waves of the blocks be populated; what
SH-MM15b and SH-MM15g leave to it is the floor's size.
...
/-- **SH-MM11a, the commit probability.** -/
...
/-- **SH-MM11b, the commit probability at wave four.** -/
...
/-- **SH-MM11e, a good coin commits the chain slot.** -/
...
bound SH-MM15 states its tail in. -/
...
the bound SH-MM15d states its tail in, where the counting lemma promises one committed candidate
...
/-- **SH-MM11i, the search under the coin.** Theorem 2's asynchronous-floor clause at period one. -/
...
/-- **SH-MM11j, the expected wait of the search at period one.** Theorem 2's expectation above an
...
/-- **SH-MM11l, a scan's control slots all miss.** The chance that no slot of a scan names a
...
/-- **SH-MM11m, the expected number of scans before one anchors.** The paper's
...
/-- **SH-MM11k, the expected wait of the search at period one, at wave four.** SH-MM11j at `4 ≤ wa`,
...
/-- **SH-MM11h, the adaptive block bound.** -/
...
/-- **SH-MM15b, the tail against an adaptive adversary.** -/
def UndecidedTailAgainst (ws wa I K : ℕ) [NeZero K] : Prop :=
  ∀ (upd : UpdateRule BlockId) (k₀ : ℕ) (known : ℕ → Validator) (d : Validator) (q s M : ℕ)
    ...
    -- then the slot stays undecided with the probability SH-MM15a gives against a fixed record
...
/-- **SH-MM15c, the tail vanishes.** -/
def UndecidedTailVanishes (K : ℕ) : Prop :=
  Tendsto (fun M : ℕ => 2 * badBlockBound Validator K ^ (M / 2)) atTop (𝓝 0)

/-- **SH-MM15d, the tail at wave four.** SH-MM15a at `4 ≤ wa`, where the counting lemma's weaker
form promises one committed candidate per populated round: the same argument, with the block
bound read at a floor of one. -/
...
/-- **SH-MM15g, almost surely against an adaptive adversary.** -/
...
/-- **SH-MM15h, every slot is decided almost surely.** -/
...
/-- **SH-MM15f, some interval is anchored almost surely.** Theorem 3 (i)'s "with probability `1`
some scan finds its anchor": SH-MM15e with the anchor in place of the decision. -/
...
/-- **SH-MM15i, a matching sequence exists.** -/
