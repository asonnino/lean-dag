...
/-!
# The `3f + 1` pair's safety — statement

The claims at the Mysticeti and Mahi-Mahi pair, read at a wavelength
function `w`: `steelheadAnchored w`, Mahi-Mahi's rule at the wave of
each slot's kind. A claim numbered like one of the generic statements
of `Safety/Statement.lean` is that statement at this rule; a claim with no
generic counterpart is this pair's alone.
...
another wave; its clauses are its own halves' laws (`BlueBottlePair/`).

Eight claims, which `Statement` bundles as nine,
since SH-MM4 is one claim about the rule and one about the relation:

* **SH-MM1a, skip excludes certificates** — Lemma 1's skip half at the wave
  of the slot's own kind;
* **SH-MM1b, certificate uniqueness** — Lemma 1's uniqueness half;
* **SH-MM1c, quorum intersection across the wave** — Lemma 2: a direct
  commit of a slot of kind `κ` at round `r` leaves a certificate in the
  causal history of every block at round `r + w κ` or above;
* **SH-MM2, agreement** — Theorem 1: two views deciding one slot reach the
  same verdict, by any routes, whether the slot's wave is `ws` or `wa`
  and whether the anchor's is;
* **SH-MM3, handover** — Corollary 1: a direct commit in one view is
  committed by every view that finds an anchor for the slot, whichever
  rule decides the anchor, and no view skips it;
* **SH-MM4, conservativity** — Theorem 5: at a constant wavelength the rule
  *is* Mahi-Mahi's, the pair read at a periodic schedule's kinds is the
  ...
  second rule;
* **SH-MM5a, chain agreement** — the chain verdicts agree across views: an
...
/-- **SH-MM1a, skip excludes certificates**, at the wave of the slot's own
kind. -/
...
/-- **SH-MM1b, certificate uniqueness**, at the wave of the slot's own
kind. -/
...
/-- **SH-MM1c, quorum intersection across the wave**: a directly committed
...
/-- **SH-MM2, agreement**: two views deciding one slot agree on the verdict,
...
/-- **SH-MM4, conservativity of the rule**: at a constant wavelength the
...
/-- **SH-MM4, conservativity of the relation at wave three**: at the
...
/-- **SH-MM5a, chain agreement**: two views agree on the chain verdict of
