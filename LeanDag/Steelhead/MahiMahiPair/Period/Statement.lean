...
/-!
# The `3f + 1` pair's period sequence — statement

The period sequence at the Mysticeti and Mahi-Mahi pair: the control
verdicts read by Mahi-Mahi's rule at `wa`, the agreed output by
`steelheadAnchored w`. A claim numbered like one of the generic statements
of `Period/Statement.lean` is that statement at these rules, with the laws
and clauses discharged by Mahi-Mahi's certificates.

What the adaptive protocol's period does across views and over time
(`steelhead.md` §5). Twenty claims:

* **SH-MM10a, agreement of the period** — Theorem 4: two views that derive
  ...
  (SH-MM10l), so the anchors agree, and the next state is a function of the
  anchor's history;
* **SH-MM10b, agreement of the output under the adaptive kinds** — the
  ...
  and SH-MM2 applies at the common wavelength and schedule. The periods are
...
* **SH-MM10c, the scan ends** — once every control slot of an interval, at
...
* **SH-MM10d, the period advances under the clause** — Theorem 3 (i): under
  ...
  by SH-MM7a at the interval's own schedule and SH-MM10c. The period is kept
  in range so that the schedules are finitely many, since the control
  set is a function of the period;
* **SH-MM10e, the failover**: Theorem 3 (i)'s last clause, as
...
* **SH-MM10i, the output stalls below an undecided slot**: a slot the view
...
* **SH-MM10j, a window resolves an asynchronous slot of every candidate**:
  ...
  one round short;
* **SH-MM10k, the control schedule enumerates the control rounds**: the
...
* **SH-MM10n, every window holds two asynchronous rounds**: the adaptive
  section's "every interval and every window" at `I ≥ 2 · maxPeriod`.
  At any period `k ≥ 1` with `2 k ≤ I`, the window of an anchor at round
  `I` or above, from `max 1 (round A − I)` to `round A` as `windowBottom`
  has it, holds two asynchronous rounds;
* **SH-MM10o, the period stays a divisor of the bound**: if the initial
  period divides `K` and the update rule keeps a period a divisor of
  `K`, so does every derived period, the failover's `1` and the
  warm-up's included; the paper's "every candidate divides maxPeriod"
  carried through the scan, which Algorithm 3 supplies at a
  power-of-two bound (SH18n);
* **SH-MM10p, the control slots carry a coin**: the protocol section's
  "the multiples of maxPeriod are asynchronous slots under every
  candidate period, so they carry a coin whatever period the scan
  fixes". At a period sequence whose every period divides `K`, every
  control slot of the scan of interval `j`, at the interval's period,
  that lies in the interval or above it is an asynchronous round of the
  adaptive schedule, led by the coin: up to the boundary it is a multiple
  of the interval's own period, above it a multiple of `K` and so of the
  period of whatever interval it falls in. With SH-MM5b, the two readings
  agree on every such slot whenever either verdict is direct;
* **SH-MM10q, the gating rule**: the adaptive section's "a slot is
  evaluated once its interval's period is known". A state for interval
  `j + 1` carries a state for interval `j`, and the scan of `j` at that
  state's period has ended, with an anchor or with none. Waiting is the
  absence of a derivation, so no round of interval `j + 1` is read at a
  period before the scan below it closes;
* **SH-MM14a, output liveness under the failover**: Theorem 3 (ii) and the
  ...
  last commit lies below it (SH-MM10i), the failover fires at each such
  anchor and the period is `1` from the first, where the run decides
  everything below it (SH-MM9a);
* **SH-MM14b, every slot is decided under the clause**: SH-MM14a with its two
  ...
  every control slot below it (SH-MM7a), so the interval has an anchor at or
  ...
  boundary, so the run the clause places there is the run of rounds SH-MM14a
...
* **SH-MM14c, output liveness from a good coin and a good run**: SH-MM14a with
  ...
  good coins above that interval are SH-MM14a's run. The form the coin's
...
`0`, is never an anchor and is asked for no verdict, so SH-MM10c's premise
...
schedule needs to be unbounded. SH-MM10a, SH-MM10c, SH-MM10d and SH-MM10g hold for
whatever wavelength the agreed output is read at, SH-MM10a for the same
wavelength on both sides; SH-MM10b, SH-MM10h, SH-MM10i and SH-MM14a to SH-MM14c read it
at the pair's, `wavelength ws wa`, on a schedule whose kinds are the
validator's own sequence's (`adaptiveKind`). SH-MM10i and SH-MM14a to SH-MM14c
...
SH-MM10a assumes `3 ≤ wa`, as SH-MM5a and SH-MM10l do, and SH-MM10b `3 ≤ ws` and
`3 ≤ wa` and a round `N` the record does not reach past; SH-MM10c, SH-MM10h and
SH-MM10i assume `2 ≤ w κ`, what the laws need to carry a verdict from the
anchor's history into the view and to keep a verdict off the slots above
the anchor; SH-MM10d assumes `1 ≤ wa`, as SH-MM7a does, `2 ≤ w κ`, a positive
interval, without which every round lies in interval `0`, and the period
in range, SH-MM10g's hypotheses; SH-MM10m assumes a positive interval too, so
that interval `0` ends at round `I`; SH-MM10e, SH-MM10g and SH-MM10k read no
record; SH-MM14a assumes `2 ≤ ws ≤ wa` and `3 ≤ wa`, as SH-MM10a does, one slot
per round and a positive interval; SH-MM14b adds the period in range and
`(c + wa) · K ≤ I`, so that a window and a run of control slots fit
inside an interval at every period; SH-MM14c asks the derived period of the
anchoring interval to lie in `[1, I]`, so that the interval holds its
first control round. SH-MM10f asks `1 ≤ k`, since at `k = 0` only round `0`
is asynchronous, and SH-MM10g concludes `1 ≤ k` from the same bound on the
initial period and the update rule. SH-MM10n asks `1 ≤ k` as SH-MM10f does and
an anchor at round `I` or above, below which the window is shorter than
`I` rounds; SH-MM10o reads no record; SH-MM10p asks a positive interval and
speaks of the control slots in the interval or above it, since the
multiples of the period below the interval are slots of the schedule
too, which no scan evaluates.
...
/-- **SH-MM10a, agreement of the period.** -/
...
/-- **SH-MM10b, agreement of the output under the adaptive kinds.** -/
...
/-- **SH-MM10d, the period advances under the clause.** -/
...
/-- **SH-MM10h, the agreed output is a prefix of the output.** -/
...
/-- **SH-MM10i, the output stalls below an undecided slot.** -/
...
/-- **SH-MM10k, the control schedule enumerates the control rounds.** -/
def ControlRounds (I K : ℕ) : Prop :=
  ∀ j k r, (∃ i, controlRound I K j k i = r) ↔
    (r ≤ (j + 1) * I ∧ r % k = 0) ∨ ((j + 1) * I < r ∧ r % K = 0)

/-- **SH-MM10l, control verdicts agree per scan.** -/
...
/-- **SH-MM10m, the first interval keeps its period.** -/
...
/-- **SH-MM10n, every window holds two asynchronous rounds.** -/
...
/-- **SH-MM10o, the period stays a divisor of the bound.** -/
def PeriodDvd (U : BlockUniverse Validator BlockId Payload) (I K wa : ℕ) [NeZero K] : Prop :=
  ∀ (coin : ℕ → Validator) (upd : UpdateRule BlockId) (k₀ : ℕ)
    (V : View Validator BlockId Payload U) (w : ℕ → ℕ) (j : ℕ) (st : ScanState),
...
/-- **SH-MM10p, the control slots carry a coin.** -/
...
/-- **SH-MM10q, an interval is evaluated only once the preceding scan has ended.** -/
def GatedByScan (U : BlockUniverse Validator BlockId Payload) (I K wa : ℕ) [NeZero K] : Prop :=
  ∀ (coin : ℕ → Validator) (upd : UpdateRule BlockId) (k₀ : ℕ)
    (V : View Validator BlockId Payload U) (w : ℕ → ℕ) (j : ℕ) (st' : ScanState),
...
/-- **SH-MM14a, output liveness under the failover.** -/
...
/-- **SH-MM14b, every slot is decided under the clause.** -/
...
/-- **SH-MM14c, output liveness from a good coin and a good run.** -/
...
def Statement : Prop :=
  ∀ (Validator BlockId Payload : Type) [Fintype Validator] [DecidableEq Validator]
    ...
    PeriodAgreement U I K wa ∧ AdaptiveAgreement U ws wa I K ∧ ScanEnds U I K wa ∧
      ...
      WarmUp U I K wa ∧ TwoAsyncRoundsInWindow I ∧ PeriodDvd U I K wa ∧
      ControlSlotsAsync (Validator := Validator) I K ∧ GatedByScan U I K wa ∧
      OutputLiveness U ws wa I K ∧
