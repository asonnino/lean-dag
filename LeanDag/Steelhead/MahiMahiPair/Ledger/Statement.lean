...
/-!
# The `3f + 1` pair's ledger — statement
...
of `Ledger/Statement.lean` is that statement at this rule; a claim with no
...
* **SH-MM13a, the committed-leader sequence is agreed** — two validators
  that settled the same prefix read off the same list, in slot order,
  which is round order since `slotRound` is monotone;
* **SH-MM13b, the ledger is agreed** — and so is the set of blocks it
  delivers, the causal histories of those leaders;
* **SH-MM13c, the ledger is monotone** — nothing already output is dropped
  as further slots settle;
* **SH-MM13d, a block enters at one slot, agreed** — the slot a block
  enters at is the same in both views, and no block enters at two slots;
* **SH-MM13e, a committed block belongs to one slot** — the integrity half:
  without it one block could be delivered by two slots.

SH-MM13a to SH-MM13d assume `2 ≤ w κ`, what Steelhead's laws need, and a
settled prefix; SH-MM13e assumes neither, since a commit names its slot
whatever the wave. They are order and integrity, not progress: under the
stall (SH-MM8) the prefix is short, and that is a liveness question.
...
/-- **SH-MM13a to SH-MM13d, the output of a settled prefix.** -/
...
/-- **SH-MM13e, integrity**: a committed block is the candidate of one slot,
