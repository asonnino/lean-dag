...
/-!
# The `3f + 1` pair's atomic broadcast — statement
...
of `Broadcast/Statement.lean` is that statement at this rule; a claim with no
...
ledger theorems (SH-MM13) and the liveness theorems make them true. Five
claims:

* **SH-MM17a, agreement** — "if one honest validator commits a block, all
  ...
  liveness half, SH-MM6b under synchrony and SH14b/SH15 under asynchrony;
* **SH-MM17b, integrity** — "each honest validator commits a block at most
...
* **SH-MM17c, validity** — "if an honest validator proposes a block, all
  ...
  prefix below is settled. That such a leader commits is SH-MM6a, that the
  prefix settles SH-MM6b; under asynchrony the committed leaders are the
  coin's (SH14a, SH15) and the delivery of a reliable block to them is
  the substrate's, which the model states as `SynchronisedOn` and not
  otherwise;
* **SH-MM17d, validity under asynchrony** — the same clause read off the
...
* **SH-MM17e, total order** — "if an honest validator commits `b` before
...
SH-MM17a and SH-MM17e assume `2 ≤ w κ`, what the laws need; SH-MM17b assumes
nothing of the wave; SH-MM17c and SH-MM17d read no wave, since a committed
...
/-- **SH-MM17d, validity under asynchrony.** -/
