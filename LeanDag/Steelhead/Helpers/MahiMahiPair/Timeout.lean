...
/-!
# Helpers — the `3f + 1` pair's mistimed leader timeout

Generated proof layer; not part of the audit surface. Each helper closes the
conjunct of the same name in `MahiMahiPair/Timeout/Statement.lean`.
-/
...
/-- **SH-MM20a, the unanimous vote round.** The ratio is the same binomial over
...
/-- **SH-MM20a, one abstention.** At the threshold `n − f` the ratio is `f / n`,
...
/-- **SH-MM20b.** The layer cake of the tail probabilities is geometric in
