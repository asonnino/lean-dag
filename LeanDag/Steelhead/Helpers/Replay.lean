...
never raises the score, and the passes are bounded by the window's
bottom and top, on any window's evidence.
-/
...
/-- **SH18k.** With the current period a candidate, the selection is the winner exactly when the
winner's score is below `1 − ε` times the current period's, and the current period otherwise. -/
theorem select_hysteresis (candidates : List ℕ) (scores : ℕ → ℚ) (current : ℕ) (epsilon : ℚ)
    (hc : current ∈ candidates) :
    (select candidates scores current epsilon ≠ current →
    ...
    (scores (best candidates scores current) < (1 - epsilon) * scores current →
      select candidates scores current epsilon = best candidates scores current) := by
  constructor
  · intro hne
    dsimp only [select] at hne ⊢
    rw [if_pos hc] at hne ⊢
...
/-- **The selection fold's invariant** over the processed candidates `P`: the incumbent scores no
worse than the current period and than every processed candidate, is the current period whenever
it scores as well, and is otherwise the largest processed candidate at its score. -/
def TieInv (scores : ℕ → ℚ) (current inc : ℕ) (P : List ℕ) : Prop :=
  scores inc ≤ scores current ∧ (∀ y ∈ P, scores inc ≤ scores y) ∧
    (scores inc = scores current → inc = current) ∧
    (inc ≠ current → ∀ y ∈ P, scores y = scores inc → y ≤ inc)

/-- A step of the selection fold keeps the invariant: a strictly better candidate replaces the
incumbent, a tie replaces it only when the incumbent is not the current period and the candidate
is the current period or larger, and otherwise the incumbent stays. -/
theorem tieInv_step {scores : ℕ → ℚ} {current inc : ℕ} {P : List ℕ}
    (h : TieInv scores current inc P) (x : ℕ) :
    TieInv scores current (if prefer scores current x inc then x else inc) (P ++ [x]) := by
  obtain ⟨h₁, h₂, h₃, h₄⟩ := h
  have hmem : ∀ y ∈ P ++ [x], y ∈ P ∨ y = x := fun y hy => by
    rcases List.mem_append.mp hy with hy | hy
    · exact Or.inl hy
    · exact Or.inr (List.mem_singleton.mp hy)
  rcases lt_trichotomy (scores x) (scores inc) with hlt | heq | hgt
  · -- strictly better: the candidate replaces the incumbent
    have hp : prefer scores current x inc = true := by
      unfold prefer; exact decide_eq_true (Or.inl hlt)
    rw [if_pos hp]
    refine ⟨le_trans hlt.le h₁, fun y hy => ?_, fun hx => ?_, fun _ y hy hyx => ?_⟩
    · rcases hmem y hy with hy | rfl
      · exact le_trans hlt.le (h₂ y hy)
      · exact le_rfl
    · exact absurd (lt_of_lt_of_le hlt h₁) (by rw [hx]; exact lt_irrefl _)
    · rcases hmem y hy with hy | rfl
      · exact absurd (lt_of_lt_of_le hlt (h₂ y hy)) (by rw [hyx]; exact lt_irrefl _)
      · exact le_rfl
  · -- a tie: the candidate replaces the incumbent only under the tie rule
    by_cases htie : inc ≠ current ∧ (x = current ∨ inc < x)
    · have hp : prefer scores current x inc = true := by
        unfold prefer; exact decide_eq_true (Or.inr ⟨heq, htie.1, htie.2⟩)
      rw [if_pos hp]
      refine ⟨heq ▸ h₁, fun y hy => ?_, fun hx => ?_, fun hxc y hy hyx => ?_⟩
      · rcases hmem y hy with hy | rfl
        · exact heq ▸ h₂ y hy
        · exact le_rfl
      · exact absurd (h₃ (heq ▸ hx)) htie.1
      · rcases hmem y hy with hy | rfl
        · have hlt : inc < x := htie.2.resolve_left hxc
          exact le_trans (h₄ htie.1 y hy (hyx.trans heq)) hlt.le
        · exact le_rfl
    · have hp : prefer scores current x inc = false := by
        unfold prefer
        refine decide_eq_false fun hor => ?_
        rcases hor with hlt | ⟨_, hne, hor⟩
        · exact absurd heq hlt.ne
        · exact htie ⟨hne, hor⟩
      rw [if_neg (by rw [hp]; exact Bool.false_ne_true)]
      refine ⟨h₁, fun y hy => ?_, h₃, fun hxc y hy hyx => ?_⟩
      · rcases hmem y hy with hy | rfl
        · exact h₂ y hy
        · exact heq.symm.le
      · rcases hmem y hy with hy | rfl
        · exact h₄ hxc y hy hyx
        · exact not_lt.mp fun hlt => htie ⟨hxc, Or.inr hlt⟩
  · -- strictly worse: the incumbent stays
    have hp : prefer scores current x inc = false := by
      unfold prefer
      refine decide_eq_false fun hor => ?_
      rcases hor with hlt | ⟨heq, _, _⟩
      · exact absurd hgt (not_lt.mpr hlt.le)
      · exact absurd hgt (not_lt.mpr heq.le)
    rw [if_neg (by rw [hp]; exact Bool.false_ne_true)]
    refine ⟨h₁, fun y hy => ?_, h₃, fun hxc y hy hyx => ?_⟩
    · rcases hmem y hy with hy | rfl
      · exact h₂ y hy
    ...
    · rcases hmem y hy with hy | rfl
      · exact h₄ hxc y hy hyx
      · exact absurd hgt (by rw [hyx]; exact lt_irrefl _)

/-- The selection fold keeps the invariant along the whole list. -/
theorem fold_tieInv (xs : List ℕ) (scores : ℕ → ℚ) (current : ℕ) :
    ∀ (inc : ℕ) (P : List ℕ), TieInv scores current inc P →
      TieInv scores current
        (xs.foldl (fun inc x => if prefer scores current x inc then x else inc) inc) (P ++ xs) := by
  induction xs with
  | nil => intro inc P h; simpa using h
  | cons x xs ih =>
    intro inc P h
    simp only [List.foldl_cons]
    have := ih _ (P ++ [x]) (tieInv_step h x)
    rwa [List.append_assoc, List.singleton_append] at this

/-- **SH18l.** The fold starts from the current period, where the invariant holds with nothing
processed, and ends with every candidate processed. -/
theorem best_tie_rule (candidates : List ℕ) (scores : ℕ → ℚ) (current : ℕ) :
    ...
    (best candidates scores current ≠ current →
      ∀ k ∈ candidates, scores k = scores (best candidates scores current) →
        k ≤ best candidates scores current) := by
  have h := fold_tieInv candidates scores current current []
    ⟨le_rfl, fun _ h => by simp at h, fun _ => rfl, fun _ _ h => by simp at h⟩
  rw [List.nil_append] at h
  exact h

/-- A candidate is a power of two with exponent at most the bound's logarithm. -/
theorem exists_pow_of_mem_candidatesUpto {K c : ℕ} (hc : c ∈ candidatesUpto K) :
    ∃ i, i ≤ Nat.log 2 K ∧ c = 2 ^ i := by
  unfold candidatesUpto at hc
  obtain ⟨i, hi, rfl⟩ := List.mem_map.mp hc
  exact ⟨i, Nat.lt_succ_iff.mp (List.mem_range.mp hi), rfl⟩

/-- **SH18n, first half.** At a power-of-two bound every candidate is a power of two with a
smaller or equal exponent, so it divides the bound. -/
theorem candidatesUpto_dvd {e c : ℕ} (hc : c ∈ candidatesUpto (2 ^ e)) : c ∣ 2 ^ e := by
  obtain ⟨i, hi, rfl⟩ := exists_pow_of_mem_candidatesUpto hc
  rw [Nat.log_pow (by norm_num)] at hi
  exact Nat.pow_dvd_pow 2 hi
...
theorem update_range (E : Evidence Validator) (C : Config Validator) (candidates : List ℕ)
    (epsilon : ℚ) {K k : ℕ} (hc : ∀ c ∈ candidates, 1 ≤ c ∧ c ≤ K) (h1 : 1 ≤ k) (hk : k ≤ K) :
    1 ≤ update E C candidates k epsilon ∧ update E C candidates k epsilon ≤ K := by
  unfold update
  rcases select_eq_or_mem candidates (score E C) k epsilon with h | h
  · rw [h]; exact ⟨h1, hk⟩
  · exact hc _ h

omit [DecidableEq Validator] F in
/-- **SH18n, second half.** The selection answers the current period or a candidate
(`select_eq_or_mem`), both divisors of the bound. -/
theorem update_dvd (E : Evidence Validator) (C : Config Validator) (epsilon : ℚ) {e k : ℕ}
    (hk : k ∣ 2 ^ e) : update E C (candidatesUpto (2 ^ e)) k epsilon ∣ 2 ^ e := by
  unfold update
  rcases select_eq_or_mem (candidatesUpto (2 ^ e)) (score E C) k epsilon with h | h
  · rw [h]; exact hk
  · exact candidatesUpto_dvd h
...
omit [Fintype Validator] [DecidableEq Validator] F [LinearOrder BlockId] in
/-- **SH18m.** A candidate is a power of two, to which an odd spacing is coprime, so SH18j
applies. -/
theorem probe_exists_of_odd {E : Evidence Validator} {C : Config Validator} {canary K period : ℕ}
    (hcan : C.canary = some canary) (hodd : Odd canary) (hmem : period ∈ candidatesUpto K)
    (hp : 2 ≤ period) (hws : 1 ≤ C.ws) (hwin : E.bottom + 2 * canary + C.ws ≤ E.top + 2) :
    0 < (probeRate E C period).2 := by
  obtain ⟨i, _, rfl⟩ := exists_pow_of_mem_candidatesUpto hmem
  exact probe_exists hcan (Nat.Coprime.pow_right i (Nat.coprime_two_right.mpr hodd)) hp hws hwin
