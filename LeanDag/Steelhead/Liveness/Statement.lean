...
* **SH6c, a crashed leader is skipped** — Theorem 2's "crashed-led
  ...
  carries;
* **SH6d, partial dissemination does not defer** — Theorem 2's remark:
...
* **SH6e, a reliable slot above the floor decides the slot** — Theorem
...
* **SH6f, the floor chain decides** — Theorem 2's chain of floors as a
  ...
  a landing whose own floor chain has committed is decided (SH6e's
  ...
  SH6g;
* **SH6g, the round-robin schedule offers a reliable run** — what the
...
* **SH6h, the floor chain reaches a reliably led landing** — Theorem 2's
  ...
  free;
* **SH6i, the floor chain reaches a reliably led landing within `b`
  hops** — Theorem 2's count as the paper states it, `b` the Byzantine
  validators: SH6h once every validator outside `T` that is not
  ...
  landings' residues are distinct (SH6h's count), and the Byzantine
...
* **SH6j, the floor chain decides within `(b + 1) · (ws + f)` rounds** —
  ...
  schedule with SH6i's crashed validators: a reliably led round lies
  within `n − |T|` rounds above any floor (SH6g), it commits (SH6a) and
  so is not passed over, so each hop lands within `ws + (n − |T|)`
  rounds of the slot below it; one of the first `b + 1` landings is
  reliably led (SH6i); and its commit decides the chain's start (SH6f).
...
* **SH6l, a reliable leader commits under the timed discipline** — the
...
* **SH9a, the drain** — Theorem 3 (ii): at any wavelength function
...
`2 ≤ ws ≤ k` and nothing of `wa`; SH9a assumes `1 ≤ w κ ≤ wa`; SH9b
...
/-- **SH6d, partial dissemination does not defer.** -/
