...
theorem fast_vote_of_sole (hrule : VotingRule U) {tx : Tx}
    (hval : T.Valid tx)
    (hsole : ∀ tx', Conflict tx tx' → ∀ b ∈ U.ids, ¬ Includes U b tx')
  ...
  have hsole' : ∀ tx', IsCandidate U b₁ (T.input tx) tx' → tx' = tx := by
    intro tx' h'
    by_contra hne
    exact hsole tx' ⟨fun h => hne h.symm, h'.2.1.symm⟩ b₁ hb₁ h'.2.2
  have hnobot : ¬ StanceIs U (U.block b₁).author (T.input tx) b₁ (some Stance.bot) := by
    ...
    · exact hsole t₂ ⟨fun h => hne h, h₂.2.1.symm⟩ e he h₂.2.2
    · exact hsole t₁ ⟨fun h => hne₁ h.symm, h₁.2.1.symm⟩ e he h₁.2.2
