...
Two qualifications, both recorded rather than fixed here.
...
*Two arcs are on separate routes by construction.* `Reactive.ReactiveCore`
is a sibling of `ViewSync` rather than a refinement — it carries `blk`
total, so production is data there — and `Drift.CatchupSync` extends
`ViewSync`, inheriting the same. Neither is reachable from `ViewPace`
without a counterpart structure, and neither is provided here.
-/
...
theorem synchronised_of_viewPace {U : BlockUniverse Validator BlockId Payload} {N R D : ℕ}
    (vp : ViewPace U (Correct : Finset Validator) N)
    (hD : DriftOn vp.built (Correct : Finset Validator) R D N) (hgst : vp.gst ≤ R)
...
Each example takes a `ViewPace` and the three network-side side conditions
— drift, GST, and the backoff — and produces the arc's conclusion. Nothing
...
variable {U : BlockUniverse Validator BlockId Payload} {N R D : ℕ}
...
example {c : ℕ} (hc : 0 < c) (hspan : SpansEligible (Validator := Validator) c)
  ...
  exact ⟨b, hk, hR, fun hN =>
    hrest U N (fun r _ hr => populated_of_viewPace vp r hr)
      (synchronised_of_viewPace vp hD hgst hbackoff) hN⟩
...
example {c : ℕ} (hc : 0 < c) (hspan : Odontoceti.SpansEligible Validator c)
  ...
  obtain ⟨b, hk, hR, hrest⟩ :=
    Odontoceti.all_decided_below_of_fairRun (BlockId := BlockId) (Payload := Payload)
      hc Finset.Subset.rfl card_correct hspan fair R k
  exact ⟨b, hk, hR, fun hN =>
    hrest U N (vp.populatedOn card_correct)
      (vp.synchronisedOn_of_converges hD hgst hbackoff) hN⟩
