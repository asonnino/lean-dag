...
/-!
# Steelhead counterexample: a complete window too short for a wave
...
* both candidates score `10`, so **Algorithm 3 keeps period `2` at every hysteresis `ε ≥ 0`**,
...
/-- **Algorithm 3 keeps period `2` at every hysteresis**: the best candidate scores what the
