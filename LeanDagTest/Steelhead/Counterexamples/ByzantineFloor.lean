...
/-!
# Steelhead counterexample: the Byzantine floor
...
/-- **Two hops of the floor chain** (SH6f's `FloorHop`): the anchor search leaves slot `0` at its
...
/-- **Every landing is led by the Byzantine validator**, so SH6f's last hypothesis, a reliably led
landing, fails at each hop. That is the whole distance between this universe and SH6f's
...
-- The side condition of SH6g's first half at the synchronous wave, and its failure at wave `4`.
...
`4m + 3`. This is SH6g's first half on data, at the `c` the synchronous wave asks for. -/
