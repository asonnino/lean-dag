...
/-!
# Steelhead counterexample: the replay on a startup window

Algorithm 3's replay does not by itself map a window in which no synchronous slot commits to
...
* **Algorithm 3 keeps period `4`**, at hysteresis `0` as at the deployment's `1/10`, and so does
...
/-- **Algorithm 3 keeps period `4`** at the anchor, at hysteresis `0` and at `1/10`. -/
