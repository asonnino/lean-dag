...
SH11h bounds the probability that every block of a set holds a bad coin when the good set of a
...
own block, checks the hypotheses on data, and reads off the bound SH11h gives the first.
...
/-- The family reads only the coins drawn before each round, SH11h's first hypothesis. -/
theorem ac_nonAnticipating : ∀ g g' (j : Fin 2) (i : Fin 1),
    (∀ j' : Fin 2, j' < j → g j' = g' j') → (∀ i' : Fin 1, i' < i → g j i' = g' j i') →
    acGood g j i = acGood g' j i := by
  decide

/-- Every good set holds two validators, SH11h's second hypothesis at `c = 2`. -/
...
/-- **The bound SH11h gives this family**: both blocks hold a bad coin with probability at most
