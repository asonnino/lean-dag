...
import LeanDag.HammerheadTwo.Window.Proof
...
/-!
# Hammerhead 2.0 witnesses — the count moves, and two runs agree
...
*outside* the range, which HH3 does not constrain. At interval `1`,
`runP1` closes two configurations (the back-off doubles, the count stays
at the floor) and `runP1'` one, exercising HH3 at two heights on a
configuration `init` does not pin. HH2, HH3 and HH7 are applied on this
data through their `holds`; HH4 and HH5 are not — see
`hammerhead-two.md` §5 on total runs.
-/
...
abbrev hhRule32 : BaseRule (Fin 4) (Fin 32) Unit := mysticeti
...
example : observed hhRule32 hhP hhLeader hhWin Usun 21 1 (by decide) (by decide) = 2 := by
  decide
example : expected hhRule32 hhP 1 = 2 := by decide
example : Aimd.rule hhRule32 hhP hhLeader hhWin 1 0 Usun 21 = (2, 0) := by decide
...
skipped slot there. -/
def run2 : PartialRun hhRule32 hhP hhLeader hhWin (Aimd.rule hhRule32 hhP hhLeader hhWin)
    Usun Vsun 1 where
...
abbrev laws32 : hhRule32.Laws := Mysticeti.holds (Fin 4) (Fin 32) Unit

/-- HH3 on `run2` and `run2'`: the two views hold one configuration `1`
and one anchor. -/
example : run2.count 1 = run2'.count 1 ∧ run2.start 1 = run2'.start 1 :=
  let h := (Agreement.holds (Fin 4) (Fin 32) Unit hhRule32 laws32 hhP hhLeader hhWin _)
    Usun Vsun Vsun' 1 1 run2 run2' 1 (by decide)
  ⟨h.2.1, h.1⟩
example : run2.anchor 0 = run2'.anchor 0 :=
  ((Agreement.holds (Fin 4) (Fin 32) Unit hhRule32 laws32 hhP hhLeader hhWin _)
    Usun Vsun Vsun' 1 1 run2 run2' 0 (by decide)).2.2.2 (by decide) |>.1

/-- HH2 on `Usun`: the smaller view holds the anchor, hence its history. -/
example : historyFrom Usun.block 21 ⊆ Vsun'.ids :=
  ((Window.holds (Fin 4) (Fin 32) Unit hhRule32 laws32).1 Usun Vsun' 21 (by decide))

/-- HH7a on the witness parameters: the rule stays in range at the values
the run meets. -/
example : 0 < (Aimd.rule hhRule32 hhP hhLeader hhWin 1 0 Usun 21).1 ∧
    (Aimd.rule hhRule32 hhP hhLeader hhWin 1 0 Usun 21).1 ≤ hhP.maxLeaders :=
  (Aimd.holds (Fin 4) (Fin 32) Unit hhRule32 hhP hhLeader hhWin).1 1 0 Usun 21
-- HH7b: below the cap, one more leader.
example : Aimd.update hhP 3 0 true = (4, 0) :=
  (Aimd.holds (Fin 4) (Fin 32) Unit hhRule32 hhP hhLeader hhWin).2.1.1 3 0 (by decide)
-- HH7c: from count `4` at back-off `1`, strictly fewer and the back-off
-- incremented; the step is `4 - 2`; at the floor, the floor.
example : (Aimd.update hhP 4 1 false).1 < 4 ∧ (Aimd.update hhP 4 1 false).2 = 2 :=
  (Aimd.holds (Fin 4) (Fin 32) Unit hhRule32 hhP hhLeader hhWin).2.2.1.1 4 1 (by decide)
example : (Aimd.update hhP 4 1 false).1 = 4 - 2 ^ 1 :=
  (Aimd.holds (Fin 4) (Fin 32) Unit hhRule32 hhP hhLeader hhWin).2.2.1.2.1 4 1 (by decide)
example : (Aimd.update hhP 1 3 false).1 = 1 :=
  (Aimd.holds (Fin 4) (Fin 32) Unit hhRule32 hhP hhLeader hhWin).2.2.1.2.2 3
-- HH7d: the rule at anchor `21`, count `2`, is the step of the integer
-- test — `96 · 4 ≤ 100 · 4` here, healthy; at interval one and anchor
-- `20` the test fails, `96 · 1 ≤ 100 · 0`.
example : Aimd.rule hhRule32 hhP hhLeader hhWin 2 0 Usun 21 =
    Aimd.update hhP 2 0 (decide (hhP.num * expected hhRule32 hhP 2 ≤
      hhP.den * observed hhRule32 hhP hhLeader hhWin Usun 21 2 (by decide) (by decide))) :=
  (Aimd.holds (Fin 4) (Fin 32) Unit hhRule32 hhP hhLeader hhWin).2.2.2 2 0 (by decide) (by decide)
    Usun 21
example : Aimd.rule hhRule32 hhP hhLeader hhWin 2 0 Usun 21 = (3, 0) := by decide
example : Aimd.rule hhRule32 hhP1 hhLeader hhWin 1 1 Usun 20 =
    Aimd.update hhP1 1 1 (decide (hhP1.num * expected hhRule32 hhP1 1 ≤
      hhP1.den * observed hhRule32 hhP1 hhLeader hhWin Usun 20 1 (by decide) (by decide))) :=
  (Aimd.holds (Fin 4) (Fin 32) Unit hhRule32 hhP1 hhLeader hhWin).2.2.2 1 1 (by decide) (by decide)
    Usun 20
-- On `Usun` at anchor `21` the window is healthy at every count: the
-- count climbs to the cap and stays there.
example : Aimd.rule hhRule32 hhP hhLeader hhWin 4 0 Usun 21 = (4, 0) := by decide

/-- HH2b on two views: the views differ, the windows do not. -/
example : historyFrom Usun.block 21 ∩ Vsun.ids = historyFrom Usun.block 21 ∩ Vsun'.ids :=
  (Window.holds (Fin 4) (Fin 32) Unit hhRule32 laws32).2 Usun Vsun Vsun' 21 (by decide) (by decide)
...
example : hhRule32.IsLeaderBlock (Sched hhLeader hhWin 1 (by decide) (by decide)) Usun 5 21 :=
...
outside range `0`, which ends at the anchor, and so unconstrained. HH3
...
def run2x : PartialRun hhRule32 hhP hhLeader hhWin (Aimd.rule hhRule32 hhP hhLeader hhWin)
    Usun Vsun' 1 where
  start := fun k => if k = 0 then 0 else 5
  count := fun k => if k = 0 then 1 else 2
  backoff := fun _ => 0
...
the same execution from `Vsun'`, closed one configuration lower. HH3 at
...
def runP1 : PartialRun hhRule32 hhP1 hhLeader hhWin (Aimd.rule hhRule32 hhP1 hhLeader hhWin)
    Usun Vsun 2 where
...
example : runP1.backoff 0 = 0 ∧ runP1.backoff 1 = 1 ∧ runP1.backoff 2 = 2 ∧ runP1.count 2 = 1 :=
  ⟨rfl, rfl, rfl, rfl⟩
-- HH3 at heights `2` and `1`: agreement on configuration `1` — not pinned by `init` —
...
example : runP1.backoff 1 = runP1'.backoff 1 ∧ runP1.start 1 = runP1'.start 1 :=
  let h := (Agreement.holds (Fin 4) (Fin 32) Unit hhRule32 laws32 hhP1 hhLeader hhWin _)
    Usun Vsun Vsun' 2 1 runP1 runP1' 1 (by decide)
  ⟨h.2.2.1, h.1⟩
example : runP1.vdct 0 2 = runP1'.vdct 0 2 :=
  ((Agreement.holds (Fin 4) (Fin 32) Unit hhRule32 laws32 hhP1 hhLeader hhWin _)
...
leader is validator `0`; HH5 applied to
...
example : run2.ledgerUpto 1 = run2'.ledgerUpto 1 :=
  ((Ledger.holds (Fin 4) (Fin 32) Unit hhRule32 laws32 hhP hhLeader hhWin _).1
    Usun Vsun Vsun' 1 1 run2 run2').2 1 (by decide)
example : run2.ledgerUpto 0 <+: run2.ledgerUpto 1 :=
  (Ledger.holds (Fin 4) (Fin 32) Unit hhRule32 laws32 hhP hhLeader hhWin _).2.1
    Usun Vsun 1 run2 0 1 (by decide)
example : (run2.ledgerUpto 1).Nodup :=
  (Ledger.holds (Fin 4) (Fin 32) Unit hhRule32 laws32 hhP hhLeader hhWin _).2.2
...
example : (runP1.ledgerUpto 2).Nodup :=
  (Ledger.holds (Fin 4) (Fin 32) Unit hhRule32 laws32 hhP1 hhLeader hhWin _).2.2
...
def run1c : PartialRun hhRule hhP1 hhLeader hhWin (constRule hhRule) U7 V7 1 where
  start := fun k => if k = 0 then 0 else 2
  count := fun _ => 1
  backoff := fun _ => 0
  anchor := fun _ => 2
...
example : run1c.count 1 = 1 ∧ run1c.backoff 1 = 0 :=
  (Conservativity.holds (Fin 4) (Fin 24) Unit hhRule hhP1 hhLeader hhWin).1 U7 V7 1 run1c 1 le_rfl
example : hhRule.Decided hhSched1 V7 2 (run1c.vdct 0 2) :=
  (Conservativity.holds (Fin 4) (Fin 24) Unit hhRule hhP1 hhLeader hhWin).2 U7 V7 1 run1c 0
