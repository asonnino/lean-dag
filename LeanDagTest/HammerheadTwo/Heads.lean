...
/-!
# Hammerhead 2.0 witnesses — the heads descent on data
...
* **Runs of heads.** Round-robin on four validators with `T = {1, 2, 3}`
  ...
  directly (`ρ := r + (5 − r % 4) % 4`) and through HH9d; six is tight
...
* **The descent producing a skip.** `hhLiveSk` — Mysticeti with `Good`
  ...
  `3, 0, 1`, all in `T`, so HH9b at `ρ = 3` decides every slot at rounds
  `0` to `2` — verdicts `some 0`, `some 5` and, for the skipped slot,
  `none`, each identified by `agree` — and commits the head of `3`; the
  application sits on the horizon exactly, `ρ = 4` does not fit. HH9a on
...
theorem hhCorrect : (Correct : Finset (Fin 4)) = {1, 2, 3} := by decide
...
theorem usun_good :
  ...
  ⟨{1, 2, 3}, by rw [hhCorrect], by decide, usun_sync, fun r h1 h2 => by
...
example : ∃ κ, 1 ≤ κ ∧ κ ≤ 4 ∧
    ∃ L, hhRule32.Decided sched1 (View.full Usun) κ (some L) := by
  obtain ⟨T, hcard, hT⟩ :=
...
example : (mysticetiLive (Validator := Fin 4) (BlockId := Fin 32) (Payload := Unit)).LiveOn
    (Sched hhLeader hhWin 2 (by decide) (by decide)) 6 :=
  MysticetiLive.holds.2 4 (by omega) (Fin 32) Unit 4 hhWin 2 (by decide) (by decide)
...
theorem usk_goodT : ∀ (S : Slots (Fin 4)) (κ : ℕ),
    ...
    ∃ L, hhRule32.Decided S (hhLiveSk.full Usk) κ (some L) := by
...
theorem hhLiveSk_descent : hhLiveSk.Descent 1 where
  goodLeaders := by
...
theorem usk_skip2 : hhRule32.Decided sched1 (hhLiveSk.full Usk) 2 none :=
  Decided.directSkip (S := sched1) (fun L hL => by have := hallSk L hL; subst this; decide)

/-- **HH9b on data.** The heads of rounds `3, 4, 5` are `T`-led, so every
...
example : ∃ v₀ v₁ v₂ L,
    hhRule32.Decided sched1 (hhLiveSk.full Usk) 0 v₀ ∧
    hhRule32.Decided sched1 (hhLiveSk.full Usk) 1 v₁ ∧
    hhRule32.Decided sched1 (hhLiveSk.full Usk) 2 v₂ ∧
    hhRule32.Decided sched1 (hhLiveSk.full Usk) (1 * 3) (some L) ∧
    v₀ = some 0 ∧ v₁ = some 5 ∧ v₂ = none ∧ L = 15 := by
  obtain ⟨hdec, L, hL⟩ :=
    (Heads.holds.1 (Fin 4) (Fin 32) Unit hhLiveSk 1 hhLeader 4 hhWin 0).2.1
    hhLiveSk_descent (Nat.succ_pos 2) Usk 1 8 {0, 1, 3}
  ...
  · exact laws32.agree _ _ (hhLiveSk.full Usk) 0 _ _ h0
      (Decided.directCommit (S := sched1) (by decide) (by decide))
  · exact laws32.agree _ _ (hhLiveSk.full Usk) 1 _ _ h1
      (Decided.directCommit (S := sched1) (by decide) (by decide))
  · exact laws32.agree _ _ (hhLiveSk.full Usk) 2 _ _ h2 usk_skip2
  · exact Option.some.inj (laws32.agree _ _ (hhLiveSk.full Usk) 3 _ _ hL
      (Decided.directCommit (S := sched1) (by decide) (by decide)))
...
example : ¬ (∀ i, i < 3 → hhLeader (3 + i) ∈ ({1, 2, 3} : Finset (Fin 4))) := by decide

/-- **HH9a on data.** The stretch `[3, 5]` of `Sched 1` on `Usk` — three direct
...
example : ∃ v₀ v₁ v₂,
  ...
  have hstretch := (Heads.holds.1 (Fin 4) (Fin 32) Unit hhLiveSk 1 hhLeader 4 hhWin 0).1
    hhLiveSk_descent sched1 Usk (hhLiveSk.full Usk) 3 5
...
example :
    ¬ (∀ i, i < 3 → sched1.slotRound i + hhLiveSk.waveLength ≤ sched1.slotRound 4) := by
  intro h
  have := h 2 (by omega)
...
theorem u44_good :
  ...
  ⟨{1, 2, 3}, by rw [hhCorrect], by decide, u44_sync, fun r h1 h2 => by
    interval_cases r <;> decide⟩
...
#print axioms LeanDag.HammerheadTwo.Heads.holds
#print axioms LeanDag.HammerheadTwo.MysticetiLive.holds
