...
sitting down to write its witness (§16).
...
```lean
structure ViewPace (U) (T : Finset Validator) (N : ℕ) where
  top : Validator → ℕ
  built : Validator → ℕ → ℕ
  timeout : ℕ → ℕ
  gst delay : ℕ
...
they serve the drift derivation below.)
...
The 319 theorems that either another module of the
