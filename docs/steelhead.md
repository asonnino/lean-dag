...
- **SH1–SH5a, safety at a wavelength function** (§3): the certificate
...
- **SH6, liveness under synchrony** (§6): a reliably led slot commits
  ...
  (SH6d), and a slot is decided once every slot from its floor up to
  some reliably led slot is decided (SH6e), or once the chain of floors
  above it reaches a reliably led landing (SH6f); and the round-robin
  ...
  which is what SH6b asks for (SH6g), and at `ws · (n − |T|) < n` the
  ...
  `ViewPace` whose timeout clears the delay (SH6l). The paper's Theorem
  2, for the decision relation.
- **SH7, chain liveness** (§4): at any schedule whose rounds strictly
  ...
  coin schedule, below any one run of `wa` good coins (SH7c). The control
  half of Theorem 3 (i).
...
- **SH9a, the drain** (§4): `wa` consecutive commits decide every slot
...
- **SH10, the scan's state** (§5): the period, the agreed output's cursor
  ...
  over the intervals the record's rounds fall in (SH10a, SH10b); the
  ...
  (SH10k, SH10l); the scan of an interval ends once its control verdicts
  are in, and under the clause at every control schedule in range it
  ends for every interval (SH10c, SH10d); an anchor below which the
  agreed output committed nothing for `I` rounds hands the next interval
  period `1` (SH10e), and the first interval's anchor keeps its period
  (SH10m); every interval and every window holds two asynchronous
  rounds, and the period stays in its range and a divisor of the bound
  (SH10f, SH10n, SH10g, SH10o); the control slots of every scan carry a
  coin under the adaptive schedule (SH10p); an interval is evaluated only
  once the scan below it has closed (SH10q); the agreed output is a prefix of
  the view's own and stalls below a slot the view leaves undecided
  (SH10h, SH10i); and a window of `I + 1` rounds resolves an asynchronous
  slot of every candidate once `I ≥ K + wa − 2` (SH10j). Theorem 4, the
...
- **SH11, the coin** (§4): with a uniform coin the chain slot of a
  ...
  `wa ≥ 4` (SH11b); the coin names a Byzantine leader with probability
  `b / n` (SH11c); over `m` rounds the probability that no round's coin
  ...
  `m = wa` (SH11d). The probability half of Theorem 3 (i), in Mathlib's
  ...
  drain below a block of good coins (SH11i), and the search waits for
  the first good block, `1 / p^wa` blocks of `wa` rounds in expectation,
  the paper's `wa / p^wa` (SH11j), at most `n^wa` blocks at `wa ≥ 4`
  (SH11k); no bound per hop of the
  search holds (§7, finding 9). A scan's `c` control slots all miss with
  probability at most `((f + b) / n)^c` (SH11l), so a scan anchors
  within `1 / (1 − ((f + b) / n)^c)` intervals in expectation, the
  asynchronous section's `1 / (1 − (1 − p)^c)` (SH11m). And Theorem 3's "with probability `1`" in the form a finite
  record admits (SH15): over the coins of `M` blocks of `wa · K` rounds,
  ...
  (SH15e), Theorem 3's "with probability `1`" itself, some interval above
  the slot's is anchored (SH15f), and every slot at
  once, each with its own sequence of records (SH15h). Both hold against
  an adversary that builds its record from the draws already made
  (SH11h, SH15b, SH15g), keeping at every round a floor of committed
  ...
  view derives, which those claims quantify over, always exists (SH15i).
...
- **SH14a, output liveness under the failover** (§5): a slot
...
- **SH17, atomic broadcast** (§3): Definition 1 clause by clause over
  ...
  there delivers it whoever led it (SH17d); that such a round exists is
...
- **SH18, the replay** (§5): Algorithm 3 as data, the window's evidence
  ...
  commit probability on the window read as a record (SH18i), a
...
- **SH20, the mistimed leader timeout** (§9): the arithmetic of the
...
| paper | here | remark |
| :--- | :--- | :--- |
| Definition 1 (atomic broadcast) | SH17, with SH6b, SH14b, SH15e | agreement, integrity and total order over settled prefixes, validity after GST with the first committed reliable leader two rounds up (SH17c) or, under asynchrony, with the first committed slot above the round by which the reliable validators have referenced the block, whoever led it (SH17d); that A1's reference rule yields such a round is taken as a hypothesis, the substrate's references sitting one round back (§7, finding 11); the "eventually" is SH6b under synchrony, SH14b under the clause and SH15e almost surely under asynchrony. The order of the blocks one commit releases is not modelled |
...
| Appendix on a mistimed leader timeout, `π(v) = 1` at `v = n` and `f / n` at `v = n − 1`, and the `1 / P₂` rounds an undecided slot waits | SH20a, SH20b | the two ratios exactly, `C(n − 1, q) / C(n, q) = (n − q) / n` at the threshold `q = n − f`; and the layer cake of the appendix's tail probabilities `(1 − p)^m`, which sums to `1 / p`. The arrival model, the minimum-quorum reference pattern and the independence between validators' orders are the appendix's assumptions, not results, and `P₁`, `Pr[V = n] = p^(n−1)` and the measured rates are outside the model (§9) |
| Adaptive section, the gating rule, "a slot is evaluated once its interval's period is known" | SH10q | a state for interval `j + 1` carries a state for interval `j` and a closed scan of `j` at that state's period, with an anchor or with none, so the derivation itself gates: waiting is the absence of a state, and no round of an interval is read at any period until the scan below it closes |
| Clause A5, the candidates fixed "before the round's coin can be learned" and the leader uniform "conditional on that history ... whatever the adversary learned from earlier coins" | SH15b, SH15g, SH11h | the arc's floor of committed candidates is a subset of `good` at each round and is a function of the draws already made (`NonAnticipating`), so the per-round bound multiplies against an adversary free to rebuild the DAG at every draw; the model does not derive the floor, which is what the clause supplies. The paper restated A5 this way on 2026-09-22; Mahi-Mahi's own arc needs no such condition, taking a DAG-shape hypothesis in place of the product |
| Theorem 4 (agreement of the period) | SH10a, SH10b, SH10k, SH10l | for any deterministic update rule, per scan: the control slots of a scan are a function of the interval's period, the period bound and the boundary (SH10k) and their verdicts agree across views (SH10l); SH10a at one wavelength and schedule on both sides, SH10b at each validator's own derived wavelength and on the schedule its own sequence names, where the sequences agree below the record's top interval and the verdicts with them |
| Appendix, the bounds `I ≥ 2 · maxPeriod` and `I ≥ maxPeriod + wa − 2`, and `1 ≤ k ≤ maxPeriod` | SH10f, SH10n, SH10g, SH10j, SH18h | two asynchronous rounds per interval, and per window of an anchor at round `I` or above, at any `k ≥ 1` with `2k ≤ I`; the period stays in range when the initial period does and the update rule keeps it there, the failover's `1` included, which Algorithm 3's replay does whenever the candidates lie in `[1, K]` (SH18h). The bound admits `I < maxPeriod + wa − 2`, where a window of `I + 1` rounds holds the decision round of none of its asynchronous slots at some anchors and of one at others (SH10j; §7, finding 8) |
| Adaptive section, "the replay is exact when the window holds no probe" | SH18i | in the part that is a theorem: at a round the window retains, the share of the `n` candidates the window marks committed is the probability that a uniform coin names a directly committed leader on the anchor's history read as a record, the paper's `c_r / n`, at `1 ≤ wa`. The anchor's term is the approximation the paper admits |
...
| Adaptive section, "the multiples of maxPeriod are asynchronous slots under every candidate period, hence defined and coin-carrying whatever the scan decides", with "every candidate divides maxPeriod" | SH18n, SH10o, SH10p | at a power-of-two bound every candidate divides it and Algorithm 3 answers a divisor of the bound from one (SH18n), so every derived period divides the bound (SH10o), and every control slot of a scan that lies in its interval or above it is an asynchronous round of the adaptive schedule, led by the coin (SH10p); with SH5b the two readings agree on every such slot whenever either verdict is direct |
...
| Appendix, "Period updates while output is stalled", and the Lean appendix's "the stall, a committed asynchronous slot deciding nothing below it" | SH8 | the argument of that paragraph as a theorem, for every `2 ≤ ws ≤ k` rather than the one period it walks through (§4) |
...
- **SH5a, chain agreement**: the chain verdicts (§4) agree across views,
...
settled, the committed-leader sequences coincide (SH13a), so do the
ledgers they deliver (SH13b), the ledger only grows as further slots
settle (SH13c), a block enters at one slot and both views name the same
one (SH13d), and a committed block is the candidate of one slot
(SH13e). Every conjunct is the anchored relation's own
...
(SH17a), a block one view delivers over a settled prefix every view
delivers over any settled prefix at least as long (from SH13b and
SH13c); integrity (SH17b), a delivered block is a block of the record, so
one its author proposed, and enters the ledger at one slot (from SH13d);
validity (SH17c), a reliable block at round `r` lies in the cone of every
...
below is settled (`reaches_of_synchronisedOn`); and total order (SH17e),
two blocks enter at the same slots in every view that settled them, so
in the same order (from SH13d). Validity under asynchrony reads the reference rule's consequence
instead of synchrony (**SH17d**): a block every reliable validator has
...
and SH15e almost surely under asynchrony. The order of the blocks one
...
is Mahi-Mahi's: agreement per scan (SH10l), liveness under the run
...
SH11b): the paper's wavelength-four trade-off. The coin lands among the
Byzantine validators with probability `b / n`, at most `f / n`
(`byzantine_prob_eq`, `byzantine_prob_le`, SH11c), the protocol
...
at `m = wa` is the `p^{wa}` a window is asked for (`runProb_ge`, SH11d).
...
SH11a, SH11b, SH11c, SH11d, SH11e, SH11f and SH11g over `commitProb`,
`runProb` and `noCommitProb`, three of the quantities `Model/Coin.lean`
defines.

**The search under the coin** (`coinOfBlocksFrom`, SH11i). Theorem 2
...
naming committed candidates is decided, by the drain (SH9a), so over `M`
...
on a known leader, which no coin governs. **SH11j**
...
falling within a wave of the good block. **SH11k**
...
(SH11b): the first `m` blocks are all bad with probability at most
...
`firstGoodInterval`, SH11l, SH11m). The asynchronous section counts the
...
`1 / (1 − (1 − p)^c)` intervals. **SH11l** (`noCommitProbOn_le`) is the
inner bound, SH11f read at a schedule rather than a stretch, since
...
at most `((f + b) / n)^c`. **SH11m** (`expected_firstGoodInterval_le`)
...
SH11j) sums the geometric series to its inverse. What the statement does
...
populated through it, the form SH11f/d already take. `undecidedProb`
...
of `n^(wa·K)` (`no_good_block_prob_le`). **SH15c**
(`undecided_tail_tendsto_zero`) is that the bound vanishes as `M` grows,
since `n − f − b ≥ 1`. **SH15d** (`undecidedProb_le_four`,
...
one committed correct candidate per populated round (SH11b), so the
...
fixed, as SH11f's are: the adversary that shapes the DAG does not see
...
**The adaptive adversary** (`NonAnticipating`, SH11h, SH15b). A record
...
the revealed history, earlier coins included. **SH11h** (`no_good_block_prob_le_adaptive`) is the block bound
...
product once the good sets read the draw. **SH15b**
(`undecidedProb_le_adaptive`) is SH15a against a strategy, at the same
bound and by the same inclusion read at the floor, and **SH15g**
(`decidedAlmostSurely_adaptive`) is SH15e over a sequence of strategies,
...
**The coin as a process** (`coinMeasure`, SH15e). What one finite record
...
set of maps the disjoint union of its members' boxes). **SH15e**
...
(`undecided_coin_le`), which vanishes (SH15c); so they are null, by
...
**SH15h** (`allDecidedAlmostSurely`) is every slot at once, each with
its own sequence of records, since the slots are countably many and the
null sets add up. **SH15f** (`anchoredAlmostSurely`) is the same
...
(`anchored_of_good_blocks`, `unanchored_coin_le`). **SH15i** (`matchingPer_matches`) supplies the period
...
the slots an earlier period left undecided: **SH9a**
...
claims that relate a schedule to the coin one clause at a time (SH14a)
take this schedule as their instance (SH15), and a sequence matching what
a view derives on it always exists (`matchingPer`, SH15i).
...
- **SH10a, agreement of the state**: two views deriving a state for
  ...
  other and SH10l forbids it, and an anchor in one view against none in
...
- **SH10f, SH10n, SH10g, SH10o, the shape of the adaptive run**: at any
  ...
  "every interval and every window" (SH10n); if the initial period lies
  ...
  divisor of `K`, so does every derived period (SH10o), the paper's
  "every candidate divides maxPeriod" carried through the scan, which
  Algorithm 3 supplies at a power-of-two bound (SH18n).
- **SH10h, SH10i, the agreed output**: every slot the agreed output
...
- **SH10k, SH10p, the control schedule enumerates the control rounds,
  ...
  schedule, led by the coin (SH10p, `controlRound_adaptive_async`): up
  ...
  carry a coin whatever period the scan fixes. With SH5b, the two
  readings agree on every such slot whenever either verdict is direct.
- **SH10l, control verdicts agree per scan**: two views agree on the
...
- **SH10m, the first interval keeps its period**: the warm-up of
  ...
  `0` or above.
- **SH14a, output liveness under the failover**: in a view that derived
  ...
  their candidates directly (SH11e's argument at the output schedule),
  and the drain SH9a decides every slot below the run. At `2 ≤ ws ≤ wa`,
...
- **SH14b, every slot is decided under the clause**: SH14a with its two
  ...
  that schedule places the run of rounds SH14a needs. Every slot whose
...
- **SH14c, output liveness from a good coin and a good run**: SH14a with
  ...
  SH14a needs. Two events at named places, each of a fixed positive
...
period or a candidate, which is what SH10o asks of an update rule. What
...
**SH6d**, the paper's "partial dissemination alone does not defer": a
...
equivocate; neither clause asks the quorum to be correct. **SH6e**,
...
validator (§7, finding 7). **SH6f** takes the chain of floors itself:
...
successor commits is decided by SH6e's argument and not skipped, so it
commits in turn and anchors the landing below it. **SH6g** is what
...
rest on the coin (SH7, SH14a) and not on the schedule. Together with SH6e
this is Theorem 2's Byzantine-led clause as a round count, `w + b + c`
rounds above the floor.

**SH6k and SH6l** say where SH6a's hypothesis comes from, and they are
...
this one, which is what the wave of three needs of an execution. SH6k is
...
**SH6h** counts the hops themselves, which is how the paper states the
...
reliably led landing within that many hops, which with SH6f decides the
...
**SH6i** is the same count with the paper's `b`, the Byzantine
...
from outside `T` is Byzantine-led; the residues are distinct as in SH6h,
...
**SH6j** (`floorChainDecidesWithinRounds`) turns the hop count into
...
1. **The control slots are fixed by rule, and the claims are per
   ...
   the run clause per scan (SH10k, SH10l, SH10d), so that a round above
   ...
   stays undecided. The probabilistic tail (SH15) accordingly reads
...
4. **Theorem 3's premise is the failover, which the replay's selector
   ...
   at the stuck slot (SH10i); a count of commits over the window, which
   never sees the stuck slot below it, cannot read that. SH10e and SH14a
...
5. **Theorem 2's ordering bound holds in expectation, not for every
   ...
   probability tending to one (SH11i, SH15), its mean, the wait for the
   first good block, at most `1 / p^wa` blocks in expectation (SH11j),
   and the ordering itself almost surely (SH15e). The deterministic part
   of Theorem 2 is the synchronous slots'
   (SH6a) and the crashed leaders' (SH6c).
6. **Lemma 3 counts on the DAG, the replay reads the window.** The
   ...
   `r`'s wave", which is SH18d's hypothesis.
7. **An equivocating Byzantine leader at the floor is the anchor.**
   ...
   from its floor up to the first commit above it is decided", SH6e's
   ...
   slot is a run of reliably led slots above its floor (SH6b, SH9a), or
   one reliably led slot above the floor with every slot between
   decided (SH6e), or a chain of floors reaching a reliably led landing
   (SH6f), and no bound in `b` alone holds on an arbitrary schedule. At
   ...
   `n − |T|` hops (SH6h), within `b` once every other validator outside
   `T` has crashed (SH6i), and a reliable leader sits within `n − |T|`
   rounds of every round with a reliable run of three past every round
   (SH6g). The bound is the schedule's, not the decision rule's, which
   is how the paper now argues it; the round count `(b + 1)(ws + f)`
   itself is SH6j, as `(b + 1) · (ws + (n − |T|))` above an unskipped
   slot at SH6i's schedule.
   ...
   proposes nothing is not distinguished from one that equivocates; SH6i
...
8. **`I ≥ 2 · maxPeriod` does not make a window hold a wave.** The
   ...
   (SH10j). The paper states both bounds, in the appendix's outline
...
9. **No per-hop bound holds for the anchor search under the coin.**
   ...
   form (SH11i): at period one a slot below a block of `wa` good coins is
...
11. **A1's reference rule lies outside the substrate.** The rule
    ...
    on it and SH17d takes its consequence, a round by which every
...
stated at `wa ≥ 5` with the counting lemma's `n − f − b` (SH15a) and at
`wa ≥ 4` with its floor of one (SH15d), the two waves the implementation
...
carry. Everything downstream is already generic over the floor, SH15d
running the same tail at a floor of one, so that one lemma would reach
the whole family. The arc runs one slot per round
(`∀ t, S.slotRound t = t` in SH10i and SH14a); the paper's introduction
...
period was reached, so SH10a's `st₁ = st₂` says nothing about the count.
...
counterexample (§3), and SH17d's conclusion on data: the Byzantine
...
adversary's answer to the first block's draw, satisfying SH11h's
...
undecided floor (§7, finding 7); the two hops of the floor chain SH6f
reads (`0` to `3` to `6`), every landing of which the Byzantine
validator leads, so that SH6f's reliably led landing is the one thing
missing; and the round-robin schedule of SH6g at `n = 4`, which leads
...
period `4`, round `4`, and the warm-up keeps period `4` (SH10m on data);
...
it derives (`rt_recovers`, SH14a on data). `ControlSlotsFromView.lean`:
...
`Axioms.lean`: the nine headline theorems, the carrier's persistence
and its liveness headline depend on the standard axioms only.

## 9. The mistimed leader timeout, SH20
...
- **SH20a, the certificate probability at the top of the vote round**
...
- **SH20b, the wait for the next direct commit** (`tsum_tail_eq_inv`):
...
LeanDag/Steelhead/
  ...
  Model/Coin.lean           commitProb, runProb, noCommitProb, noCommitProbOn, goodIntervals,
                            firstGoodInterval, blockRound, coinOfBlocks,
  ...
  Safety/Statement.lean     SH1–SH5a        Safety/Proof.lean
  Liveness/Statement.lean   SH6–SH9a        Liveness/Proof.lean
